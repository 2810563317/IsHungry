		...
		<div class="cart">
			<div class="content" @click="toggleList">
				...
				<div class="content-right" @click.stop.prevent="pay">
					<div class="pay" :class="payClass">{{payDeac}}</div>
				</div>
			</div>
			<div class="ball-container">
				<transition-group name="drop"  v-on:before-enter="beforeEnter" v-on:enter="enter"  v-on:after-enter="afterEnter">
					<div v-for="(ball,index) in balls" v-bind:key="index" v-show="ball.show" class="ball">
						<div class="inner inner-hool"></div>
					</div>
				</transition-group>
			</div>
			<transition name="fold">
				<div class="shopcart-list" v-show="listShow">
					...
					<div class="list-content" ref="listContnet">
						<ul>
							<li class="food border-1px" v-for="food in selectFoods" v-if="food.count">
	  ...
	  computed: {
		  ...
		  listShow(){
		  	...
		  	if(show){
		  		this.$nextTick(() => {
		  			if(!this.scroll){
		  				this.scroll =new BScroll(this.$refs.listContnet,{
		  					click:true
		  				});
		  			}else{
		  				this.scroll.refresh();
	  ...
	  methods:{
	  	...
	  	beforeEnter(el){
				...
				while(count--){
					...
					if(ball.show){
						let reac = ball.el.getBoundingClientRect();//获取元素相对于视口的位置
						//获取偏移量
						let x=reac.left-32;
						let y=-reac.top+22;
						el.style.display='';
						...
						let inner = el.getElementsByClassName('inner-hool')[0];
